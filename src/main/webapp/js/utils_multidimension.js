/**
 * Created by yidu on 2015/3/20.
 * 支持多维度
 */

//地图对象
var map = [];
var mapId = ["map1", "map2", "map3", "map4", "map5", "map6"];
var calendarId = ["detailTrendsCycle1","detailTrendsCycle2","detailTrendsCycle3",
    "detailTrendsCycle4","detailTrendsCycle5","detailTrendsCycle6"];
//谷歌地图与bing地形地图
...
//各种过滤控件
var polygonConrtol = [],
    navControl,provinceSelectionControl,pointSelectionControl;
...
function initUIs() {
    for (var i = 0; i < mapId.length; i++) {
        mapLayer[i] = new OpenLayers.Layer.Google(
            "Google Physical", // the default
            {numZoomLevels: 20}
        );
        hybridLayer[i] = new OpenLayers.Layer.Bing({
            name: "Hybrid",
            key: apiKey,
            type: "AerialWithLabels"
        });
        if(i == 0){
            map[0] = new OpenLayers.Map(mapId[0], {
...
//                controls: [new OpenLayers.Control.Zoom(), new OpenLayers.Control.Navigation]
            ...
            map[i] = new OpenLayers.Map(mapId[i], {
                projection: new OpenLayers.Projection("EPSG:4326"),
                displayProjection: new OpenLayers.Projection("EPSG:900913"),
                //        layers: [hybridLayer,mapLayer],
                layers: [mapLayer[i]],
        ...
        heatmapLayer[i] = new OpenLayers.Layer.Heatmap("heatmap"+i, map[i], mapLayer[i],
            {
                opacity: 0,
                radius: 10,
                scaleRadius: true,
        ...
        heatmapLayer[i].projection = "EPSG:900913";

        map[i].addLayers([heatmapLayer[i]]);

        locationLayer[i] = createLocationLayer(i);
        map[i].addLayer(locationLayer[i]);

        createPolygonControl(i);
    }
 ...
 * 创建点层
 */
function createLocationLayer(index){
    ...
    locationLayer[index] = new OpenLayers.Layer.Vector('location',{styleMap:vector_style_map,renderOptions: {zIndexing: true}});
    var features = [];
    locationLayer[index].addFeatures(features);
    locationLayer[index].events.on({
        "featureselected":function(e){
            ...
            popup.autoSize = true;
            popup.minSize = new OpenLayers.Size(300,180);
            e.feature.popup = popup;
            map[index].addPopup(popup);
            var dragPopup = new OpenLayers.Control.DragPopup(popup);
            map[index].addControl(dragPopup);
            createHourDetail(id,200,100,"station_code="+ e.feature.data.station_code);
        },
        "featureunselected":function(e){
            map[index].removePopup(e.feature.popup);
            e.feature.popup.destroy();
            e.feature.popup = null;
        }
    });
    return locationLayer[index];
}
...
function createPolygonControl(index){
	polygonLayer[index] = new OpenLayers.Layer.Vector("Polygon Layer"+index, {
	   eventListeners: {
			"featureadded": featureAddedListener,
		   "sketchstarted":sketchStartedListener
		}
	});
	map[index].addLayer(polygonLayer[index]);
	map[index].addControl(new OpenLayers.Control.MousePosition());
	polygonConrtol[+index] = new OpenLayers.Control.DrawFeature(polygonLayer[+index],
		OpenLayers.Handler.Polygon)
	map[index].addControl(polygonConrtol[index]);
}
function sketchStartedListener(event){
    for(var i = 0; i < mapId.length; i ++){
	    polygonLayer[i].removeAllFeatures();
    }
}
...
function featureAddedListener(event){
    ...
    //TODO map[0]
	var projectTo = map[0].getProjectionObject(); //The map projection (Spherical Mercator)
	...
	$.ajax({
		...
		success:function(data){
			...
			buildHM(newData);
			//调整中心点
            for(var i = 0; i < mapId.lentgh; i ++)
			    map[i].zoomToExtent(event.feature.geometry.getBounds(), true);
		}
	});
}
/**
...
//    data.max
    ...
    for(var i = 0; i < mapId.length; i ++){
        transformedData[i] = { max: data.max , min:0,data: [] };
        var length = data.data.length;
        while(length --){
            if(i == 0){
                transformedData[i].data.push({
                    lonlat: new OpenLayers.LonLat(data.data[length].longitude, data.data[length].latitude).
                        transform(new OpenLayers.Projection("EPSG:4326"),  map[0].getProjectionObject()),
                ...
                transformedData[i].data.push({
                    lonlat: new OpenLayers.LonLat(data.data[length].longitude, data.data[length].latitude).
                        transform(new OpenLayers.Projection("EPSG:4326"),  map[0].getProjectionObject()),
                    count: data.data[length].so2_ave
                });
            }else if(i == 2){
                transformedData[i].data.push({
                    lonlat: new OpenLayers.LonLat(data.data[length].longitude, data.data[length].latitude).
                        transform(new OpenLayers.Projection("EPSG:4326"),  map[0].getProjectionObject()),
                    count: data.data[length].pm10_ave
                });
            }else if(i == 3){
                transformedData[i].data.push({
                    lonlat: new OpenLayers.LonLat(data.data[length].longitude, data.data[length].latitude).
                        transform(new OpenLayers.Projection("EPSG:4326"),  map[0].getProjectionObject()),
                    count: data.data[length].co_ave
                });
            }else if(i == 4 ){
                transformedData[i].data.push({
                    lonlat: new OpenLayers.LonLat(data.data[length].longitude, data.data[length].latitude).
                        transform(new OpenLayers.Projection("EPSG:4326"),  map[0].getProjectionObject()),
                    count: data.data[length].o3_ave
                });
            }else if(i == 5){
                transformedData[i].data.push({
                    lonlat: new OpenLayers.LonLat(data.data[length].longitude, data.data[length].latitude).
                        transform(new OpenLayers.Projection("EPSG:4326"),  map[0].getProjectionObject()),
                    count: data.data[length].no2_ave
                });
            }
        }
    }
    ...
    for(var i = 0; i < mapId.length; i ++) {
        heatmapLayer[i].setDataSet(transformedData[i]);
    }
}
...
function displayPoints(){
	$.ajax({
        url:"year.do",
        data: "timeList=2014&airList=pm25_ave&airList=no2_ave&airList=o3_ave&airList=pm10_ave&airList=so2_ave&airList=co_ave",
//		url:"cities.do",
		type:"post",
		dataType:"json",
		success:function(data){
...
function buildLocationLayer(data){
    ...
    for(var j = 0; j < mapId.length; j ++){
        ...
        if(j == 0){
            for ( var i = 0; i < data.length; ++i) {
                features[j].push(new OpenLayers.Feature.Vector(
            ...
            for ( var i = 0; i < data.length; ++i) {
                features[j].push(new OpenLayers.Feature.Vector(
                        new OpenLayers.Geometry.Point(data[i].longitude,data[i].latitude).
                            transform(new OpenLayers.Projection("EPSG:4326"),  map[0].getProjectionObject()),
                        {value:data[i].so2_ave,station_code:data[i].station_code}
                    )
                );
            }
        }else if(j == 2){
            for ( var i = 0; i < data.length; ++i) {
                features[j].push(new OpenLayers.Feature.Vector(
                        new OpenLayers.Geometry.Point(data[i].longitude,data[i].latitude).
                            transform(new OpenLayers.Projection("EPSG:4326"),  map[0].getProjectionObject()),
                        {value:data[i].pm10_ave,station_code:data[i].station_code}
                    )
                );
            }
        }else if(j == 3){
            for ( var i = 0; i < data.length; ++i) {
                features[j].push(new OpenLayers.Feature.Vector(
                        new OpenLayers.Geometry.Point(data[i].longitude,data[i].latitude).
                            transform(new OpenLayers.Projection("EPSG:4326"),  map[0].getProjectionObject()),
                        {value:data[i].co_ave,station_code:data[i].station_code}
                    )
                );
            }
        }else if(j == 4){
            for ( var i = 0; i < data.length; ++i) {
                features[j].push(new OpenLayers.Feature.Vector(
                        new OpenLayers.Geometry.Point(data[i].longitude,data[i].latitude).
                            transform(new OpenLayers.Projection("EPSG:4326"),  map[0].getProjectionObject()),
                        {value:data[i].o3_ave,station_code:data[i].station_code}
                    )
                );
            }
        }else{
            for ( var i = 0; i < data.length; ++i) {
                features[j].push(new OpenLayers.Feature.Vector(
                        new OpenLayers.Geometry.Point(data[i].longitude,data[i].latitude).
                            transform(new OpenLayers.Projection("EPSG:4326"),  map[0].getProjectionObject()),
                        {value:data[i].no2_ave,station_code:data[i].station_code}
                    )
                );
            }
        }
    ...
    for(var i = 0; i < mapId.length; i ++) {
        locationLayer[i].removeAllFeatures();
        locationLayer[i].addFeatures(features[i]);
    }
...
//		.interpolate("monotone")
    ...
    if(filteredData.length != 0) {
        ...
        cmd = "allDayTrendsByStationCodes2.do" + "?" + station_codes;
    }else{
        cmd = "allDayTrendsByStationCodes2.do";
    }
        ...
        var attrs = color.domain().map(function(name) {
            return {
                ...
                values: data.map(function(d) {
                    return {time_point: d.time_point, value: +d[name]};
                })
            };
        });
 ...
 */
function cycleTime(){
    for(var i = 1 ; i <= calendarId.length; i ++) {
        $("#detailTrendsCycle"+i).empty();
        $("#detailTrendsCycle"+i).html("<div id=\"monthAvg"+i+"\"></div><div id=\"monthDetail"+i+"\"></div>");
        createMonthDetail(i);
    }
//    createMonthAvg();
...
function createMonthDetail(index){
//    index = index -1;
	var width = 270,
		height = 100;
    var cellWidth = 5,
...
//        percent = d3.format("4d"),
	...
	var svg = d3.select("#monthDetail"+index).selectAll("svg")
	...
	d3.json(cmd, function(error, json) {
		var data = d3.nest()
			...
			.rollup(function(d) {
                if(index == 1) return d[0].pm25_ave;
                else if(index == 2) return d[0].so2_ave;
                else if(index == 3) return d[0].pm10_ave;
                else if(index == 4) return d[0].co_ave;
                else if(index == 5) return d[0].o3_ave;
                else if(index == 6) return d[0].no2_ave;
            })
			.map(json);
