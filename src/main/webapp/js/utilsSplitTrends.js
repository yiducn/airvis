...
//	}
	...
	d3.json("dayTrendsByCodes.do"+cities, function(error, data) {
...
//            console.log(name);
	...
	function brushed() {
        if(brush.empty()){
            linearTimeDetail(0, x.domain());
        }else {
            linearTimeDetail(0, brush.extent());
        }
//        console.log(brush);
...
//		focus.select(".x.axis").call(xAxis);
    ...
    function brushend(){
        linearTimeDetail(1,brush.extent);
    }
}
 ...
 * 根据brush的粒度，呈现detail
 */
function linearTimeDetail(tag, brush){
    ...
    var x = d3.time.scale().range([0, width]),
        y = d3.scale.linear().range([height, 0]).domain([0,  150]);
    var xAxis = d3.svg.axis().scale(x).orient("bottom"),
...
//        .interpolate("monotone")
        .x(function (d) {
//            console.log("x:"+parseDate(d.time_point)+":"+x(parseDate(d.time_point)));
            return x(parseDate(d.time_point));
        })
        .y(function (d) {
//            console.log("y:"+d.avg_time+":"+y(d.avg_time));
            return y(d.avg_time);
        });

    if(brush == null && tag == null) {
...
//        console.log(brush);
        ...
        console.log("todo: show day value");
    }
}
...
//        .attr("fill",monthAvgColor);
	...
	d3.json("dayTrendsByCodes.do", function(error, json) {
		var data = d3.nest()
