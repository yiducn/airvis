...
public class LUdcmp {
    ...
    public LUdcmp(final double[][] a) {
        ...
        for (i=0;i<n;i++) {
            ...
            if (big == 0.0) throw new IllegalArgumentException("Singular matrix in LUdcmp");
            vv[i]=1.0/big;
        }
        for (k=0;k<n;k++) {
            big=0.0;
    ...
    public final static double[][] buildMatrix(final int nn, final int mm, final double a) {
    ...
    public final static double[][] buildMatrix(final int nn, final int mm, final double[] b) {
        double[][] v = new double[nn][mm];
        for(int i=0; i<nn; i++)
            for(int j=0; j<mm; j++)
                v[i][j] = b[i*mm+j];
    ...
    public final static int[][] buildMatrix(final int nn, final int mm, final int[] b) {
        int[][] v = new int[nn][mm];
        for(int i=0; i<nn; i++)
            for(int j=0; j<mm; j++)
                v[i][j] = b[i*mm+j];
    ...
    public final static double[][] buildMatrix(final double[][] b) {
        int nn = b.length;
        int mm = b[0].length;
        double[][] v = new double[nn][mm];
        for(int i=0; i<nn; i++)
